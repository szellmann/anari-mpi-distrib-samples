...
namespace util {
  ...
  MappedFile::MappedFile(const std::string &fname)
    : fm(fname)
    , view((const char *)fm.data(), fm.nbytes())
  {
  }
  size_t MappedFile::tellg() const {
    return pos;
  }
  void MappedFile::seek(size_t p) {
    pos = p;
  }
  size_t MappedFile::read(char *buf, size_t len) {
